...
int main(int argc, char** argv) {
  ...
  if (as_iiv != -1) {
    for (auto &sd : p_model->CONFIG->drug_db()
                        ->at(0)
  ...
  if (as_ec50 != -1) {
    p_model->CONFIG->EC50_power_n_table()[0][0] =
        pow(as_ec50, p_model->CONFIG->drug_db()->at(0)->n());
  }
  ...
  if (genotypes.empty()) {
    min_genotype_id = 0;
    max_genotype_id =
        static_cast<int>(Model::CONFIG->number_of_parasite_types()) - 1;
  ...
  for (auto genotype_id = min_genotype_id; genotype_id <= max_genotype_id;
       genotype_id++) {
    std::stringstream ss;
...
void create_cli_option(CLI::App &app) {
  app.add_option("-t", therapies,
                 "Get efficacies for range therapies [from to]");
  app.add_option("-g", genotypes,
