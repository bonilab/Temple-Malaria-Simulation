  ...
  try {
    ...
    for (const auto &entry : node) {
      ...
      if (locus < 0) {
        ...
        throw std::invalid_argument(
            "Locus must be greater than or equal to zero");
      }
      if (mutant_allele < 0) {
        LOG(ERROR) << "The locus must be greater than or equal to zero.";
        throw std::invalid_argument(
            "Mutant allele must be greater than or equal to zero");
      }
    ...
    LOG(ERROR) << "Unrecoverable error parsing YAML value in "
               << IntroduceMutantEvent::EventName << " node: " << error.msg;
    exit(1);
  }
}
...
std::vector<Event*> PopulationEventBuilder::build_introduce_mutant_raster_event(
  ...
  try {
    ...
    for (const auto &entry : node) {
      ...
      auto* event = new IntroduceMutantRasterEvent(time, locations, fraction,
                                                   locus, mutant_allele);
      VLOG(1) << "Adding " << event->name() << " start: " << start_date
              << " with " << locations.size() << " locations"
              << ", locus: " << locus << ", mutant_allele: " << mutant_allele
              << ", fraction: " << fraction;
      events.push_back(event);
    }
    return events;
    ...
    LOG(ERROR) << "Unrecoverable error parsing YAML value in "
               << IntroduceMutantRasterEvent::EventName
               << " node: " << error.msg;
    exit(EXIT_FAILURE);
