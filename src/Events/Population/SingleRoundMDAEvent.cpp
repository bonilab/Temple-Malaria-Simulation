...
void SingleRoundMDAEvent::execute() {
  ...
  for (std::size_t loc = 0; loc < Model::CONFIG->number_of_locations(); loc++) {
    ...
    const std::size_t number_indidividuals_in_location =
        all_persons_in_location.size();
    unsigned int number_of_individuals_will_receive_mda =
        Model::RANDOM->random_poisson(fraction_population_targeted[loc]
