...
void SpatialData::generate_distances() const {
    // Find first valid raster for cell size
    AscFile* reference = nullptr;
    for (const auto& raster : data) {
        if (raster) {
            reference = raster.get();
            break;
        }
    }

    if (!reference) {
        throw std::runtime_error("No raster data available for distance calculation");
    }
    ...
    for (std::size_t from = 0; from < locations; from++) {
        ...
        for (std::size_t to = 0; to < locations; to++) {
            distances[from][to] =
                std::sqrt(std::pow(reference->CELLSIZE
                                       * (db[from].coordinate->latitude
                                          - db[to].coordinate->latitude),
                                 2)
                          + std::pow(reference->CELLSIZE
...
int SpatialData::get_district(int location) {
    ...
    if (location < 0 || location >= Model::CONFIG->number_of_locations()) {
        throw std::out_of_range(
            fmt::format("{} called with invalid location: {}", __FUNCTION__, location));
    }
    ...
    if (coordinate == nullptr) {
        throw std::runtime_error(
            fmt::format("{} called with location {} having null coordinate", __FUNCTION__, location));
    }

    // Check if coordinates are within raster bounds
    auto* districts = data[SpatialFileType::Districts].get();  // Use .get()
    if (coordinate->latitude < 0 || 
        coordinate->latitude >= districts->NROWS ||
        coordinate->longitude < 0 || 
        coordinate->longitude >= districts->NCOLS) {
        throw std::out_of_range(
            fmt::format("{} called with location {} having out of bounds coordinates ({}, {})", 
...
void SpatialData::refresh() {
    ...
    // std::cout << "Checking number of locations: " << Model::CONFIG->number_of_locations() << std::endl;
    if (Model::CONFIG->number_of_locations() == 0) { 
