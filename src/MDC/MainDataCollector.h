 ...
 *
 * Definition of the model data collector class. Note that age_class and
 * age_group are used fairly interchangeably.
 *
 * NOTE (2023-08-09, RJZ) Removed concepts related to the useful therapeutic
 * life (UTF) as well as any code that was involved with the older way of
 * tracking treatment failures. This work was tracked on GitHub under ticket #20
 * and also resulted in the removal of the end_of_time_step accounting function;
 * AdaptiveCyclingStrategy, MFTRebalancingStrateg, and
 * NovelDrugSwitchingStrategy; along with modifications to the ConsoleReporter,
...
class MainDataCollector {
  ...
  // The total number of clinical episodes that have occurred in a location
  // since data recording started
  ...
  // The total number of mutation events that occurred in a location since data
  // recording started
  ...
  PROPERTY_REF(DoubleVector2,
               last_10_blood_slide_prevalence_by_location_age_class)

  PROPERTY_REF(DoubleVector2,
               last_10_fraction_positive_that_are_clinical_by_location)

  PROPERTY_REF(
      DoubleVector3,
      last_10_fraction_positive_that_are_clinical_by_location_age_class)

  PROPERTY_REF(
      DoubleVector3,
      last_10_fraction_positive_that_are_clinical_by_location_age_class_by_5)
  ...
  // The total number of treatments completed by location and therapy, this
  // should equal successes plus failures
  PROPERTY_REF(IntVector2, monthly_treatment_complete_by_location_therapy);

  // Monthly number of treatment failures (i.e., treatment unsuccessfully
...
private:
  ...
  // Zero out the population statistics tracked by
...
public:
  ...
  // Reset monthly tracking variables after the reporters had a chance to access
  // them.
  void monthly_update();
  ...
  // Record one birth at the given location index, this value will be reset each
  // month
  void record_1_birth(const int &location);

  // Record one death, note that this is independent of recording a malaria
  // death
  ...
  // Record one malaria death, note that this only records the cause (i.e.,
  // malaria) and record_1_death still needs to be called to ensure proper
  // metrics
  ...
  // Records one case in which a treatment was administered but failed due to
  // patient death or failure to clear
  void record_1_treatment_failure_by_therapy(const int &location,
                                             const int &age_class,
                                             const int &therapy_id);

  // Records one case in which a treatment was successful upon follow-up
  void record_1_treatment_success_by_therapy(const int &location,
                                             const int &age_class,
                                             const int &therapy_id);

  void update_after_run();

  // Calculate the artemisinin mono-therapy use (AMU) and artemisinin
  // combination therapy failure rate (ATF)
  [[maybe_unused]] void record_AMU_AFU(
      Person* person, Therapy* therapy,
      ClonalParasitePopulation* clinical_caused_parasite);

  double get_blood_slide_prevalence(const int &location, const int &age_from,
                                    const int &age_to);
