...
void AgeBandReporter::initialize(int job_number, const std::string &path) {
  ...
  pfpr_reporter.setGlobally(
      el::ConfigurationType::Filename,
      fmt::format("{}age_band_pfpr_{}.csv", path, job_number));
  ...
  cases_reporter.setGlobally(
      el::ConfigurationType::Filename,
      fmt::format("{}age_band_cases_{}.csv", path, job_number));
  ...
  // Determine when we should start running, we want the last year of data,
  ...
  // we are writing to two different immutable streams
  ...
  for (auto district = 0; district < districts; district++) {
    ...
    for (auto ac = 0; ac < age_classes; ac++) {
      pfpr << ((prevalence[district][ac] != 0)
                   ...
                   : 0)
           << Csv::sep;
      cases << prevalence[district][ac] << Csv::sep << population[district][ac]
            << Csv::sep;
    }
    pfpr << Csv::end_line;
    cases << Csv::end_line;
