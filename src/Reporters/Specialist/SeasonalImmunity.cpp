...
void SeasonalImmunity::monthly_report() {
  ...
  std::vector<RunningMedian<double>> median_theta(lookup_allocation,
                                                  RunningMedian<double>());
    ...
    for (auto hs = 0; hs < Person::NUMBER_OF_STATE - 1; hs++) {
      for (unsigned int ac = 0; ac < age_classes; ac++) {
        for (auto &person : index->vPerson()[location][hs][ac]) {
          ...
          // Update the multiclonal count if there is more than one parasite
          // clone present
  ...
  for (int zone = 0; zone < lookup_allocation; zone++) {
    ss << Model::SCHEDULER->current_time() << Csv::sep << zone << Csv::sep
